...
pub trait Cog: Send + Sync {
    ...
    async fn on_member_update(&self, _ctx: &Context, _old: Option<Member>, _new: Option<Member>, _event: &GuildMemberUpdateEvent) {}
    ...
    async fn on_role_delete(&self, _ctx: &Context, _guild_id: GuildId, _role_id: RoleId, _role: Option<Role>) {}
    async fn on_thread_create(&self, _ctx: &Context, _thread: &GuildChannel) {}
    async fn on_voice_state_update(&self, _ctx: &Context, _old: Option<VoiceState>, _new: &VoiceState) {}
...
impl CogManager {
    ...
    pub async fn dispatch_member_update(&self, ctx: &Context, old: Option<Member>, new: Option<Member>, event: &GuildMemberUpdateEvent) {
        for cog in &self.cogs { cog.on_member_update(ctx, old.clone(), new.clone(), event).await; }
    }

    pub async fn dispatch_member_ban(&self, ctx: &Context, guild_id: GuildId, banned_user: &User) {
        for cog in &self.cogs { cog.on_member_ban(ctx, guild_id, banned_user).await; }
    }

    pub async fn dispatch_member_unban(&self, ctx: &Context, guild_id: GuildId, unbanned_user: &User) {
        for cog in &self.cogs { cog.on_member_unban(ctx, guild_id, unbanned_user).await; }
    }

    pub async fn dispatch_channel_create(&self, ctx: &Context, channel: &GuildChannel) {
        for cog in &self.cogs { cog.on_channel_create(ctx, channel).await; }
    }

    pub async fn dispatch_channel_delete(&self, ctx: &Context, channel: &GuildChannel) {
        for cog in &self.cogs { cog.on_channel_delete(ctx, channel).await; }
    }

    pub async fn dispatch_role_create(&self, ctx: &Context, role: &Role) {
        for cog in &self.cogs { cog.on_role_create(ctx, role).await; }
    }

    pub async fn dispatch_role_delete(&self, ctx: &Context, guild_id: GuildId, role_id: RoleId, role: Option<Role>) {
        for cog in &self.cogs { cog.on_role_delete(ctx, guild_id, role_id, role.clone()).await; }
    }

    pub async fn dispatch_thread_create(&self, ctx: &Context, thread: &GuildChannel) {
        for cog in &self.cogs { cog.on_thread_create(ctx, thread).await; }
    }

    pub async fn dispatch_voice_state_update(&self, ctx: &Context, old: Option<VoiceState>, new: &VoiceState) {
        for cog in &self.cogs { cog.on_voice_state_update(ctx, old.clone(), new).await; }
    }
